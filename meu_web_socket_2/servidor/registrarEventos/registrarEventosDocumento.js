    ...
    async ({ nomeDocumento, nomeUsuario }, devolverTexto) => {
      ...
      if (documento) {
        ...
        if (!conexaoEncontrada) {
          ...
          //socket.to().emit() envia para todos exceto para o próprio usunaŕio conectado
          io.to(nomeDocumento).emit(
            "usuarios_no_documento",
            usuariosNoDocumento
          );
          //io.to().emit() envia para todos incluindo o remetent
      ...
      socket.on("disconnect", () => {
        if (socket.data.usuarioEntrou) {
          ...
          //io.to().emit() envia para todos incluindo o remetente
        }
      });
    }
  );
